...
void playGame(int gridSize, int numPlayers, int numRounds) {
    ...
    for (int round = 1; round <= numRounds; round++) {
        ...
        while ( players->movesPredicted < 1 && players->movesPredicted > 9){ // boucle de vérification de l'entrée de prédiction
            printf("\n\n\nEntrée invalide. Veuillez entrer un entier\n\n ");
            for (int i = 0; i < numPlayers; i++) {
                printf("Joueur %d - Prédiction de coups pour atteindre la cible %c :\n ", i+1, players[i].target);
                scanf("%d", &players[i].movesPredicted);
            }
        }
